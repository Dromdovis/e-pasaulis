...
export default function BreadcrumbsFix({ className = '' }: { className?: string }) {
  ...
  const getPageName = (path: string): string => {
    // For cart, directly return the literal string value instead of using the translation key
    if (path === 'cart') {
      return 'Cart'; // English, you can detect language and return appropriate literal
    }
    ...
    const pageNames: Record<string, TranslationKey> = {
      ...
      'users': 'users',
      'products': 'products',
      // Add more pages as needed but exclude 'cart'
    };
